...
class PairsApp {
    constructor() {
        this.activeTours = [];
        this.currentPairs = [];
        this.tourSelect = document.getElementById('tourSelect');
    ...
    async loadActiveTours() {
        try {
            ...
            const latestTour = this.activeTours.reduce((latest, tour) => tour.id > latest.id ? tour : latest);
