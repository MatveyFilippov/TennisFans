...
import { TourResponse, TourPlayerPointsResponse } from '../types/index.js';

export class TournamentBoard {
    private container: HTMLElement;
    private tourSelect: HTMLSelectElement;
    private tours: TourResponse[] = [];

    constructor(containerId: string, tourSelectId: string) {
        this.container = document.getElementById(containerId)!;
        this.tourSelect = document.getElementById(tourSelectId) as HTMLSelectElement;
    ...
    private async loadTours() {
        try {
            ...
            if (this.tours.length > 0) {
                const latestTour = this.tours.reduce((latest, tour) =>
